...
// Simulated fetch function for jobs data
const fetchJobsData = () => {
  ...
  return (
    <div className={`p-8 min-h-screen ${isDarkMode ? "bg-gray-800 text-white" : "bg-white text-black"}`}>
      ...
      <div className={`flex justify-between items-center shadow-md p-6 rounded-lg mb-6 ${isDarkMode ? "bg-gray-900" : "bg-white"}`}>
        ...
        <button
          className={`${isDarkMode ? "bg-blue-600 text-white hover:bg-blue-500" : "bg-blue-500 text-white hover:bg-blue-400"} px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 shadow-md`}
          onClick={() => exportElementToPDF("report-content", "summary-report.pdf")}
          disabled={loading}
      ...
      {/* Tabs Navigation */}
      <div className="flex space-x-4 mb-6" role="tablist" aria-label="Summary tabs">
      ...
      <div
        className={`shadow-lg rounded-lg p-6 ${isDarkMode ? "bg-gray-900" : "bg-white"}`}
        id="report-content"
      >
        {loading ? (
          <p className="text-center text-gray-500" role="alert" aria-live="polite">
            Loading data...
          </p>
        ) : error ? (
          <p className="text-center text-red-500" role="alert" aria-live="assertive">
            {error}
          </p>
        ) : (
          <>
            ...
            {activeTab === "jobStatus" && (
              <section
                role="tabpanel"
                id="tab-panel-jobStatus"
                aria-labelledby="tab-jobStatus"
              >
