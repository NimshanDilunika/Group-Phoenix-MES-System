    ...
    const payload = {
      ...
      filters: Object.keys(filters).reduce((acc, key) => {
        if (key.endsWith('_desc')) {
          ...
          // Only send description if the corresponding checkbox is true
          acc[key] = filters[baseKey] ? filters[key] : "";
        } else if (typeof filters[key] === 'boolean') {
          acc[key] = filters[key];
        } else if (!key.endsWith('_desc') && typeof filters[key] === 'string') {
          acc[key] = filters[key];
        }
        return acc;
      }, {}),
      ...
      if (jobCardId) {
        try {
          ...
          const fetchedFilters = {
            oil_filter: data.oil_filter_status || false,
            oil_filter_desc: data.oil_filter || "",
            fuel_filter: data.fuel_filter_status || false,
            fuel_filter_desc: data.fuel_filter || "",
            air_filter: data.air_filter_status || false,
            air_filter_desc: data.air_filter || "",
            battery_charge: data.battery_charge_status || false,
            battery_charge_desc: data.battery_charge || "",
            oil: data.oil_status || false,
            oil_desc: data.oil || "",
          ...
          alert("Failed to fetch job card details.");
        }
      } else {
