...
const Home = () => {
  ...
  const jobs = [
    ...
    { id: 4, title: "Generator Repair", company: "ABC Company", date: "20 March 2024", status: "In Process", service: "Generator Services" },
    { id: 5, title: "Solar Installation", company: "SolarTech", date: "25 March 2024", status: "Cancelled", service: "Solar Systems" },
    { id: 6, title: "AC Installation", company: "HomeCare", date: "30 March 2024", status: "In Process", service: "Ai Conditioningr"},
    { id: 7, title: "HVAC System Setup", company: "ThermoTech", date: "01 April 2024", status: "In Process", service: "HVAC" },
    { id: 8, title: "Solar Battery Backup", company: "Renew Power", date: "12 April 2024", status: "Todo", service: "Solar Systems" },
    ...
    { id: 26, title: "Electric Vehicle Charger Installation", company: "EV Solutions", date: "29 July 2024", status: "In Process", service: "Solar Systems" },
    ...
    { id: 29, title: "Home Battery Backup", company: "Renew Power", date: "17 August 2024", status: "In Process", service: "Home Energy" },
    { id: 30, title: "Solar Water Pump Installation", company: "SolarTech", date: "24 August 2024", status: "In Process", service: "Solar Systems" },
    { id: 34, title: "Generator Installation", company: "ABC Company", date: "05 March 2024", status: "Pending", service: "Solar Systems" },
  ];

  // Count jobs by service
  const countJobsByService = jobs.reduce((acc, job) => {
    // If the service already exists in the accumulator, increment the count
    if (acc[job.service]) {
      acc[job.service]++;
    } else {
      // If the service doesn't exist, set its count to 1
      acc[job.service] = 1;
    }
    return acc;
  }, {});
    ...
    <div className="bg-gray-800 space-y-8 p-6 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 cursor-pointer">
        {services.map((service, index) => (
          <div
            key={index}
            className={`service-card ${selectedService === service.name ? 'bg-blue-600 border-blue-500' : 'bg-gray-800 border-gray-700'} 
              text-white rounded-lg p-4 shadow-md border-2 hover:border-blue-500 transition-all duration-300 flex items-center space-x-3`}
            onClick={() => setSelectedService(service.name)}
          >
            {/* Change icon color based on selection */}
            <div className={`${selectedService === service.name ? 'text-gray-300' : 'text-blue-500'} transition-colors duration-300`}>
              {service.icon}
            </div>
            <div>
      ...
      <div className="status-filter bg-gray-800 p-6 rounded-lg shadow-lg">
        ...
        {statuses.map((status, index) => (
          <div
            key={index}
            className={`status-card grid place-items-center bg-gray-700 text-white p-4 rounded-lg shadow-md hover:bg-gray-700 transition-all duration-200 hover:scale-103 transform 
              ${selectedStatus === status.status ? 'bg-blue-600 border-gray-700' : 'bg-gray-800 border-gray-700'} 
              border-2 hover:border-blue-500 flex items-center space-x-3`}
            onClick={() => setSelectedStatus(status.status)}
          >
            {status.icon}
            <span className="text-gray-400 mt-2">{status.text} - {status.jobs} jobs</span>
          </div>
        ))}
      </div>
      ...
      {/* Filtered Jobs List */}
      <h2 className="text-xl font-semibold text-white mb-4 mt-13">
