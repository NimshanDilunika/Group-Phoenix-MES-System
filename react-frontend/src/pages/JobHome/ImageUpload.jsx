            ...
            <motion.div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => !uploading && setShowPopup(false)}>
              <motion.div className={`rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl ${isDarkMode ? "bg-gray-800" : "bg-white"}`} initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={e => e.stopPropagation()}>
                ...
                <div className="flex justify-end gap-3">
                  <button onClick={() => { setShowPopup(false); setFiles([]); setDescription(""); }} disabled={uploading} className={`px-6 py-2.5 rounded-lg transition ${uploading ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-gray-300 hover:bg-gray-400 text-gray-700"}`}>Cancel</button>
                  <button onClick={handleUpload} disabled={!files.length || uploading} className={`px-6 py-2.5 rounded-lg shadow transition ${!files.length || uploading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"} text-white`}>
